...
import { useAppModuleCreatePageController } from "./controller/useAppModuleCreatePageController";

type AppModuleCreateV1Props = {
  appId: string;
};

export function AppModuleCreateV1({ appId }: AppModuleCreateV1Props) {
  const { t } = usePreferences();
  const { app, buildCode, error, isLoadingApp, isSubmitting, slugify, submit } =
    useAppModuleCreatePageController(appId);

  const [name, setName] = useState("");
  const [codeOverride, setCodeOverride] = useState<string | null>(null);
  const [status, setStatus] = useState<"active" | "inactive">("active");

  const moduleSlug = codeOverride ?? slugify(name);
  const fullCode = buildCode(moduleSlug);
  ...
  return (
    <div className="space-y-6">
      <AuthCenterHeader
        description={t("authz.moduleCreate.description")}
        title={app ? `${app.name} · ${t("authz.moduleCreate.title")}` : t("authz.moduleCreate.title")}
      />
          ...
          <div className="flex items-center gap-3 border-b border-[var(--dashboard-border-soft)] px-6 py-5">
            ...
            <div>
              <h2 className="text-base font-semibold text-[var(--dashboard-text)]">
                {t("authz.moduleCreate.formTitle")}
              </h2>
            ...
            <label className="block">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    setCodeOverride(slugify(e.target.value) || null)
                  }
