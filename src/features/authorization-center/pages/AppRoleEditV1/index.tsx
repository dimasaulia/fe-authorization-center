...
import { useRoleEditController } from "../RoleEditV1/controller/useRoleEditController";

type AppRoleEditV1Props = { appId: string; roleId: string };

export function AppRoleEditV1({ appId, roleId }: AppRoleEditV1Props) {
  ...
  } = useRoleEditController({ roleId, returnPath: routes.appRoles(appId) });
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--dashboard-border)] bg-[var(--dashboard-panel)] shadow-[0_1px_3px_var(--dashboard-shadow)]">
            ...
            <div className="space-y-5 px-6 py-6">
              ...
              <div className="flex items-center gap-3">
                <input checked={isSystem} className="h-4 w-4 rounded border-[var(--dashboard-border-soft)] accent-[var(--dashboard-accent)]" id="is_system_app_edit" onChange={(e) => setIsSystem(e.target.checked)} type="checkbox" />
                <label className="text-sm text-[var(--dashboard-muted-strong)]" htmlFor="is_system_app_edit">{t("authz.roleCreate.isSystemLabel")}</label>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-[var(--dashboard-border-soft)] px-6 py-4">
              <a className="inline-flex h-11 items-center justify-center rounded-xl border border-[var(--dashboard-border-soft)] bg-[var(--dashboard-field)] px-5 text-sm font-semibold text-[var(--dashboard-muted-strong)] transition hover:bg-[var(--dashboard-panel-subtle)]" href={routes.appRoles(appId)}>
                {t("authz.form.cancel")}
              </a>
              <button
